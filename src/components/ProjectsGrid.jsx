...
const ProjectsGrid = ({ jsonPath = '/src/data/projects.json' }) => {
  ...
  return (
    <section className="min-h-screen flex items-start justify-center pt-4 sm:pt-8 bg-white relative overflow-hidden">
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mt-8">
          {visibleProjects.map((project, idx) => (
            <div key={idx} className="group relative overflow-hidden rounded-xl sm:rounded-2xl bg-gray-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
              <a href={project.url} className="block aspect-[3/4] w-full overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 w-full p-3 sm:p-4 md:p-6 lg:p-8">
                  <span className="inline-block mb-1 sm:mb-2 text-xs sm:text-sm font-medium text-white/80 bg-black/30 px-2 sm:px-3 md:px-4 py-1 rounded-full">
                    {project.category}
                  </span>
                  <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl text-white mb-1">
                    {project.title}
                  </h3>
